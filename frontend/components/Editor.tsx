...
export const Editor = React.forwardRef<any, EditorProps>(({ 
  ...
  const fontFamily = isMonospace ? '"JetBrains Mono", "Fira Code", monospace' : 'Merriweather, serif';
  ...
  const toolbarBg = settings.theme === 'light' ? 'bg-white border-b border-stone-200 shadow-sm' : 'bg-stone-900 border-b border-stone-800 shadow-sm';
  const summaryBg = settings.theme === 'light' ? 'bg-white border-b border-stone-200' : 'bg-stone-900 border-b border-stone-800';
  const inputBg = settings.theme === 'light' ? 'bg-stone-50 border-stone-300 text-stone-900' : 'bg-stone-950 border-stone-800 text-stone-300';
  const textMuted = settings.theme === 'light' ? 'text-stone-500' : 'text-stone-500';
  const footerBg = settings.theme === 'light' ? 'bg-white border-t border-stone-200' : 'bg-stone-900 border-t border-stone-800';

  return (
    <div className={`flex flex-col h-full w-full overflow-hidden relative ${editorContainerBg}`}>
      ...
      <div className={`flex-none z-20 xl:hidden ${toolbarBg}`}>
         <div className="h-14 flex items-center justify-between px-4">
            ...
            <div className="flex items-center space-x-2">
               <div className={`flex items-center rounded-md p-1 space-x-1 ${settings.theme === 'light' ? 'bg-stone-100' : 'bg-stone-800'}`}>
                   <span className={`text-[10px] font-bold uppercase px-2 ${textMuted}`}>Chapter AI</span>
                   <div className={`w-px h-4 ${settings.theme === 'light' ? 'bg-stone-300' : 'bg-stone-700'}`}></div>
                   <Button size="sm" variant="ghost" className="text-xs h-7" onClick={() => onAiAction('chapter', 'extend')} disabled={isAiLoading} icon={<Wand2 size={12}/>} theme={settings.theme}>Extend</Button>
                   <Button size="sm" variant="ghost" className="text-xs h-7" onClick={() => onAiAction('chapter', 'rewrite')} disabled={isAiLoading} icon={<FileEdit size={12}/>} theme={settings.theme}>Rewrite</Button>
               </div>
            </div>
         </div>
      </div>
      ...
      {isSummaryOpen && (
        <div className={`flex-none p-4 animate-in slide-in-from-top-2 z-10 shadow-lg ${summaryBg}`}>
            <div className="max-w-4xl mx-auto">
                <div className="flex justify-between items-center mb-2">
                    <label className={`text-xs font-bold uppercase tracking-wider ${textMuted}`}>Chapter Summary</label>
                    <div className="flex space-x-2">
                        <Button size="sm" variant="secondary" className="text-xs h-6 py-0 px-2" onClick={() => onAiAction('summary', 'update')} disabled={isAiLoading} icon={<RefreshCw size={10}/>} theme={settings.theme}>AI Update</Button>
                        <Button size="sm" variant="secondary" className="text-xs h-6 py-0 px-2" onClick={() => onAiAction('summary', 'rewrite')} disabled={isAiLoading} icon={<PenLine size={10}/>} theme={settings.theme}>AI Rewrite</Button>
                    </div>
                </div>
                <textarea
            ...
            <div className="p-4 animate-in slide-in-from-bottom-2 duration-300">
               <div className="flex items-center justify-between mb-3 px-1">
                 <div className="flex items-center space-x-2 text-amber-500">
                    ...
                    <span className="text-xs font-bold uppercase tracking-wider">Choose a continuation</span>
                 </div>
                 <button onClick={() => onAcceptContinuation('')} className={`${textMuted} hover:text-stone-800 text-xs`}>Dismiss</button>
               </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-full max-h-[40vh] overflow-y-auto pr-1 custom-scrollbar">
                 {continuations.map((option, idx) => (
                   <div key={idx} onClick={() => onAcceptContinuation(option)} className={`group relative p-5 rounded-lg border cursor-pointer transition-all hover:shadow-lg hover:-translate-y-0.5 ${settings.theme === 'light' ? 'bg-stone-50 border-stone-200 hover:bg-white hover:border-amber-300' : 'bg-stone-800 border-stone-700 hover:bg-stone-750 hover:border-amber-500/50'}`}>
                      <div className={`font-serif text-lg leading-relaxed ${settings.theme === 'light' ? 'text-stone-800' : 'text-stone-300 group-hover:text-stone-100'}`}>{option}</div>
                   </div>
                 ))}
               </div>
            </div>
