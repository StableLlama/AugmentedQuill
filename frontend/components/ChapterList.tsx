...
export const ChapterList: React.FC<ChapterListProps> = ({
  ...
  const bgClass = isLight ? 'bg-stone-50 border-stone-200' : 'bg-stone-900 border-stone-800';
  const textHeader = isLight ? 'text-stone-500' : 'text-stone-400';
  const btnHover = isLight ? 'hover:bg-stone-200 text-stone-500 hover:text-stone-700' : 'hover:bg-stone-800 text-stone-500 hover:text-stone-300';
  
  const itemActive = isLight ? 'bg-white border-amber-400 shadow-sm' : 'bg-stone-800 border-amber-500/50 shadow-sm';
  const itemInactive = isLight ? 'bg-transparent border-transparent hover:bg-stone-100' : 'bg-transparent border-transparent hover:bg-stone-800';
  ...
  return (
    <div className={`flex flex-col h-full border-r ${bgClass}`}>
      <div className={`p-4 border-b flex justify-between items-center sticky top-0 z-10 ${bgClass} ${isLight ? 'border-stone-200' : 'border-stone-800'}`}>
        <h2 className={`text-sm font-semibold uppercase tracking-wider ${textHeader}`}>Chapters</h2>
        <button 
          onClick={onCreate}
