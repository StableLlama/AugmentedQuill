    ...
    return (
      <div className={`p-4 border-b shadow-sm ${containerClass}`}>
        <div className="space-y-3">
          <input
            className={`w-full text-lg font-bold border-b p-2 rounded-t focus:outline-none focus:border-amber-500 ${inputClass}`}
            value={editTitle}
            onChange={(e) => setEditTitle(e.target.value)}
            placeholder="Story Title"
          />
          <textarea
            className={`w-full text-sm rounded p-2 focus:ring-1 focus:ring-amber-500 focus:outline-none border ${inputClass}`}
            rows={3}
            value={editSummary}
          ...
          <div className="flex items-center space-x-2">
            ...
            <input
              className={`flex-1 text-xs border-b p-1 focus:outline-none focus:border-amber-500 ${inputClass}`}
              value={editTags}
