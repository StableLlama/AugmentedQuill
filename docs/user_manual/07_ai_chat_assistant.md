...
| **Attach files** (<img src="assets/clipboard.svg" alt="Clipboard icon" width="16" height="16" style="vertical-align:text-bottom;" />)                                 | Adds file attachments to your message. Click the <img src="assets/clipboard.svg" alt="Clipboard icon" width="16" height="16" style="vertical-align:text-bottom;" /> attach button (or drag & drop files onto the textarea) to pick one or more files. Each attachment appears as a chip showing the file name and size; click a chip to remove it (with confirmation). Text-like files (`.txt`, `.md`, `.json`, `.xml`, `.csv`, `.yml/.yaml`, and other text formats) are read as UTF-8 and sent inline; other files (including images, for multimodal models) are base64-encoded and sent as binary. Sending requires text or at least one attachment. |
