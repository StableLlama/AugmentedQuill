...
Every DeLorean jump is also stored as a **Time Travel** sourcebook entry, and that is what draws the **timeline panel** on the left. Each jump gets its own **arrow**, and every jump that opens a new branch is connected to the timeline it left with a **spawn arc** — a short horizontal connector followed by a right-angle curve down into the new lane. The branch scenes live on their own lanes (the 1955 past, the 2015 future, the alternate 1985, the fixing trip back to 1955, and the Old West of 1885) instead of tangling with `main`.
