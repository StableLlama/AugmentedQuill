...
/**
 * Purpose: Shared project-seeding helpers for the documentation E2E suites.
 *
 * These helpers build an isolated AugmentedQuill data directory (machine
 * config, projects registry, and rich demo projects) so that Playwright
 * configs that boot a full backend + frontend can test user-visible features
 * against deterministic, reproducible data.  They intentionally never touch
 * real user data under `data/` (see AGENTS.md "Test Data Safety").
 *
 * The demo novel exercises the writing/metadata/sourcebook/search/scenes UI;
 * the series project exercises books; the Back to the Future project
 * exercises the Convergence Map time-travel rendering.
 */
...
export const DEMO_PROJECT_NAME = 'The Undrawn Valley';
export const SERIES_PROJECT_NAME = 'The Signal Fire';
export const BTTF_PROJECT_NAME = 'Back to the Future';

// Repo root (this file lives at src/frontend/tests/e2e/support).
...
const MOCKUP_DIR = path.resolve(REPO_ROOT, 'docs/user_manual/assets/mockup');
...
/**
 * Write a minimal, valid machine.json that points the app at a local mock LLM
 * server so providers are "working" and AI actions stay enabled.
 */
export function writeMachineConfig(configDir: string, mockLlmPort: number): void {
...
/** Write an empty projects registry (backend rescans the projects dir). */
export function writeProjectsRegistry(configDir: string): void {
  fs.mkdirSync(configDir, { recursive: true });
  fs.writeFileSync(
...
/** 1x1 PNG pixel data for a flat color (base64). */
...
/**
 * Create a rich novel project on disk (write story.json + chapter files
 * before the backend scans the projects folder).
 */
 ...
 * so the books UI has multiple expanded volumes.
 */
...
/**
 * Create the time-travel (Back to the Future) project through the real
 * backend seeding service, so the Convergence Map rendering is exercised
 * with realistic data.  Requires AUGQ_* env vars pointing at tmpDir.
 */
function createDemoTimeTravelProject(tmpDir: string, configDir: string): void {
  execSync(
    `${REPO_ROOT}/venv/bin/python -m augmentedquill.testing.back_to_the_future`,
    {
      cwd: REPO_ROOT,
      env: {
        ...process.env,
        AUGQ_USER_DATA_DIR: tmpDir,
        AUGQ_PROJECTS_ROOT: path.join(tmpDir, 'projects'),
...
/**
 * Seed every demo project (novel, series, time travel) into tmpDir and
 * persist a meta file so specs/teardown can locate the temp dir.
 */
export function seedDemoProjects(
  ...
  createDemoNovelProject(path.join(tmpDir, 'projects', DEMO_PROJECT_NAME));
  createDemoSeriesProject(path.join(tmpDir, 'projects', SERIES_PROJECT_NAME));
  createDemoTimeTravelProject(tmpDir, configDir);
...
/** Read the persisted meta file written by seedDemoProjects. */
export function readDemoMetaFile(metaPath: string): {
  tmpDir: string;
  demoProject: string;
  seriesProject: string;
  bttfProject: string;
} {
